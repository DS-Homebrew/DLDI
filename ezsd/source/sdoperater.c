...
bool SD_initial()
{
	...
	SD_SendCommand(55,0);			// ﾖｸﾊｾﾏﾂﾒｻｸ・鑅ﾇappﾃ・・
	ret = SD_R16Response(pres);
	if((!ret) || pres[0] != 55)
		return false; 

	SD_SendCommandRS3(41, pres, 0x0);		//ｼ・箏醵ｹｷｶﾎｧ
	do
	{
	...
	do
	{
		SD_SendCommand(3,0);		//ｽ・tand by ﾗｴﾌｬ｣ｬｲ｢ｵﾃｵｽsdｿｨﾗｴﾌｬ
		ret = SD_R16Response(pres);
	...
	ret = SD_SendCommandRS(16,0x200) ;	//ﾉ雜ｨﾒｻｸlockﾎｪ512ｴ｡
