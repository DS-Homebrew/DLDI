...
static void waitSDState( u8 state )
{
  ...
  do
  {
    ioRpgSendCommand(cmd,4,4,&ready);
    ready&=0xff;
  } while(ready!=state);
}
...
void sddReadBlocks( u32 addr, u32 blockCount, void * buffer )
{
  ...
  u32 sdCommand[2] = { 0xab030011, address }; // input read address here
  sdCommand[1] = address;
  if( 1 == blockCount ) {
    sdCommand[0]=0xab030011;
...
void sddWriteBlocks( u32 addr, u32 blockCount, const void * buffer )
{
  ...
  u32 address = _isSDHC ? addr : (addr << 9);
  if( 1 == blockCount ) {
    u32 sdWriteBlock[2] = { 0xab050018, address };
    sendSDCommandR0( sdWriteBlock );
    ioRpgPushData( pBuffer, 512 );
    waitSDState( 0x00 );
    pBuffer += 512;
  } else {
    u32 sdWriteBlocks[2] = { 0xab060019, address };
    ...
    ALIGN(4) u8 response[4];
    u32 sdStopTransmission[2] = { 0xab01000C, 0x00000000 };
