...
static u32 SCDSSDHC_SDHostSetMode(u8 sdio, u32 parameter, u8 response_type, u32 latency) {
    ...
    return cardExt_RomReadData4Byte(command, SCDSSDHC_CTRL_READ_4B | MCCNT1_LATENCY1(latency));
}
...
static void SCDSSDHC_SDHostSetRegister(u8 bits) {
    ...
    cardExt_RomReadData4Byte(command, SCDSSDHC_CTRL_READ_4B);
    waitByLoop(0x300);
}
...
static u32 SCDSSDHC_SRAMReadData(u32 address) {
    ...
    return cardExt_RomReadData4Byte(command, SCDSSDHC_CTRL_READ_4B);
}
...
bool SCDSSDHC_SDInitialize(void) {
    ...
    cardExt_RomReadData4Byte(0x6600000000000000ull, 0xA7586000);
